...
from amp_accum import AmpAccum
...
from part3.modern_model import GPTModern
...
def main():
    ...
    sched = LRScheduler(optim, warmup_steps=warmup, total_steps=total_steps, base_lr=args.lr)

    amp = AmpAccum(optim, accum_steps=args.grad_accum_steps, amp=args.mixed_precision)
    ...
    if is_tb(logger):
        try:
            ...
            log_graph_tb(logger, model, ex_x.to(device), ex_y.to(device))
        except Exception:
            pass
