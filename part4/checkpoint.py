...
def is_tb(logger):
    return hasattr(logger,"w") is not None

def _log_hparams_tb(logger, args, total_steps):
    if not is_tb(logger): return
    try:
        h = dict(
...
def log_graph_tb(logger,model,xb,yb):
    if hasattr(logger,"graph") is None:return
    ...
    try:
        ...
        ans=_TensorOnly(model).to(xb.device())
        logger.graph(ans,(xb,yb))
    except Exception:
        pass
...
def _maybe_log_attention(logger, model, xb, step: int, every: int = 100):
    ...
    """
    if not is_tb(logger) or step == 0 or (step % every):
        return
    try:
        import torch
...
def _extract_config_from_model(model) -> dict:
    ...
    try:
        ...
        try:
            from part3.swiglu import SwiGLU  # optional
        except Exception:
            class SwiGLU: pass
