...
class CausalSelfAttentionModern(nn.Module):
    def __init__(self, n_embd: int, n_head: int, dropout: float = 0.0,
        ...
        if rope and self.rope_cache is None:
            self.rope_cache=RopeCache(head_dim=self.d_head,max_pos=max_pos)
    ...
    def forward(self,x:torch.Tensor,kv_cache:KVCache|None=None,start_pos:int=0):
        ...
        if self.use_rope:
            ...
            q=apply_single_rope(q,cos,sin)
            k=apply_single_rope(k,cos,sin)

        # Grouped QKVs for GQA
