...
sys.path.append(str(_P(__file__).resolve().parents[1]/'part4'))
try:
    ...
    _HAS_BPE = False
sys.path.append(str(_P(__file__).resolve().parents[1]/'part3'))
try:
    from part3.tokenizer import ByteTokenizer
except Exception:
    ByteTokenizer = None
...
class SFTCollator:
    ...
    def collate(self, batch: List[Tuple[str,str]]):
        ...
        for prompt, response in batch:
            prefix_text = format_prompt(prompt).replace('</s>','')
