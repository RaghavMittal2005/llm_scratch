...
from curriculum import CurriculumLength as LengthCurriculum
...
def main():
    ...
    p.add_argument('--bpe_dir', type=str, default='../part4/runs/part4-demo/tokenizer') # assumes tokenizer exists from Part 4
    args = p.parse_args()
    ...
    for it in items[:3]:
        print(f"PROMPT: {it.instruction}\nRESPONSE: {it.response}\n{'-'*40}")

    # Curriculum over (prompt,response)
    tuples = [(it.instruction, it.response) for it in items]
