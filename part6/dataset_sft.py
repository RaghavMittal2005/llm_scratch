...
def load_tiny_hf(split: str = "train[:200]", sample_dataset: bool = False) -> List[Example]:
        ...
        try:
            ...
            for row in ds:
                ...
                if inp:
                    instr = instr + "\n" + inp
                if instr and out:
                    items.append(Example(instruction=instr, response=out))
        except Exception:
            pass
    if not items:
        # fallback tiny list
        ...
        items = [Example(instruction=p, response=r) for p,r in seeds]
    return items